...
function contact_form_render_field( $field ) {
	...
	if ( $field['type'] == 'email' ) {
		$r .= "\n<div>\n";
		$r .= "\t\t<label for='".esc_attr($field_id)."' class='grunion-field-label ".esc_attr($field['type']) . ( contact_form_is_error($field_id) ? ' form-error' : '' ) . "'>" . htmlspecialchars( $field['label'] ) . ( $field['required'] ? '<span>'. __("(required)", CBP_APP_TEXT_DOMAIN) . '</span>' : '' ) . "</label>";
		$r .= "\t\t<input type='text' name='".esc_attr($field_id)."' id='".esc_attr($field_id)."' value='".esc_attr($field_value)."' class='".esc_attr($field['type'])."'/>\n";
		$r .= "\t</div>\n";
	} elseif ( $field['type'] == 'textarea' ) {
		$r .= "\n<div>\n";
		$r .= "\t\t<label for='".esc_attr($field_id)."' class='".esc_attr($field['type']) . ( contact_form_is_error($field_id) ? ' form-error' : '' ) . "'>" . htmlspecialchars( $field['label'] ) . ( $field['required'] ? '<span>'. __("(required)", CBP_APP_TEXT_DOMAIN) . '</span>' : '' ) . "</label>";
		...
		$r .= "\t<div><label class='". ( contact_form_is_error($field_id) ? ' form-error' : '' ) . "'>" . htmlspecialchars( $field['label'] ) . ( $field['required'] ? '<span>'. __("(required)", CBP_APP_TEXT_DOMAIN) . '</span>' : '' ) . "</label>";
...
function contact_form_validate_field( $field ) {
	...
	if ( $field['required'] && $field['type'] == 'email' ) {
		if ( !is_email( $field_value ) ) {
			...
			$contact_form_errors->add( $field_id, sprintf( __( '%s requires a valid email address' , CBP_APP_TEXT_DOMAIN), $field['label'] ) );
		}
		...
		$contact_form_errors->add( $field_id, sprintf( __('%s is required', CBP_APP_TEXT_DOMAIN), $field['label'] ) );
	}
...
function contact_form_shortcode( $atts, $content ) {
	...
	$r .= "\t\t<input type='submit' value='" . __( "Submit" , CBP_APP_TEXT_DOMAIN) . "' class='pushbutton-wide btn'/>\n";
	$r .= "\t\t$nonce\n";

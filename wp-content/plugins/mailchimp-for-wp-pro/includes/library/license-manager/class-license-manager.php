			...
			if( $result ) {
				...
				if( $result->license === 'valid' ) {
					...
					if( isset( $result->error ) && $result->error === 'no_activations_left' ) {
						...
						$this->set_notice( sprintf( __('Your license has expired. You must <a href="%s">extend your license</a> in order to use it again.', $this->product->get_text_domain() ), 'https://dannyvankooten.com/mailchimp-for-wordpress/checkout/?edd_license_key=' . esc_attr( $this->get_license_key() ) . '&download_id=899', false ) );
					} else {

...
if( ! $lists || empty( $lists ) ) {
	printf( __( 'No lists found, %sare you connected to MailChimp?%s', 'mailchimp-for-wp' ), '<a href="'. admin_url( 'admin.php?page=mc4wp-pro' ) .'">', '</a>' );
} else { ?>
	<ul id="mc4wp-lists">
		<?php foreach($lists as $list) {
			?><li><label><input type="checkbox" name="mc4wp_form[lists][<?php echo esc_attr( $list->id ); ?>]" value="<?php echo esc_attr( $list->id ); ?>" data-list-groupings="<?php echo esc_attr( json_encode( $list->interest_groupings ) ); ?>" data-list-fields="<?php echo esc_attr( json_encode( $list->merge_vars ) ); ?>" <?php checked( array_key_exists( $list->id, $form_settings['lists'] ), true ); //echo 'checked="checked"'; ?>> <?php echo esc_html( $list->name ); ?></label></li><?php
		} ?>
	</ul>
	<?php
} ?></p>

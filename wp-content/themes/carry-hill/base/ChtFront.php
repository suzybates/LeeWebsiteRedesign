...
class ChtFront
{
    ...
    public static function addScript($script, $args = array())
    {
        ...
        if ($args['template'] && is_array($args['template'])) {
            ...
            if (in_array($pageTemplate, $args['template']) || (is_single() && in_array('single', $args['template']))) {
                new ChtScript('front', array($script), $args['dependencies'], $args['version'], $args['inFooter']);
            }
            return;
        }
    ...
    public static function addStylesheet($path, $args = array())
    {
        ...
        if ($args['template'] && is_array($args['template'])) {
            ...
            if (in_array($pageTemplate, $args['template']) || (is_single() && in_array('single', $args['template']))) {
                new ChtStyle('front', array($path), $args['dependencies'], $args['version'], $args['media'], $args['conditional']);
            }
            return;
        }
        new ChtStyle('front', array($path), $args['dependencies'], $args['version'], $args['media'], $args['conditional']);
...
//    }
    ...
    {
        if ($type == 'glider-top') {
            return get_post_meta($postId, CHT_APP_PREFIX . 'glider-sidebar-top', true);
        }
        if ($type == 'glider-bottom') {
            return get_post_meta($postId, CHT_APP_PREFIX . 'glider-sidebar-bottom', true);
        }
        if ($type == 'bottom-widgets') {
            return get_post_meta($postId, CHT_APP_PREFIX . 'sidebar-bottom-widgets', true);
        }
        return false;
    }
}

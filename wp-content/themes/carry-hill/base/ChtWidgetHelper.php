<?php
if (!class_exists( 'ChtTOWWidget' )):
/**
 * @author Doctor Krivinarius <krivinarius@gmail.com>
 */
class ChtWidgetHelper extends WP_Widget
{
    protected $_attrs;
    ...
    protected function formElementText($name, $value, $description = null)
    {
        ?>
        <p>
            <label for="<?php echo $this->get_field_id($name); ?>"><?php echo $this->translate(Utils::titleize($name) . ':'); ?></label> 
    ...
    protected function formElementSelect($name, $options, $selectedOption)
    {
        ?>
        <p>
            <label for="<?php echo $this->get_field_id($name); ?>"><?php echo $this->translate('Width:'); ?></label>
            <select name="<?php echo $this->get_field_name($name); ?>">
                <?php foreach ($options as $value): ?>
                    <option <?php echo Utils::maybeSelected($value == $selectedOption) ?> value="<?php echo $value; ?>" title="<?php echo $value; ?>"><?php echo $value; ?></option>
                <?php endforeach; ?>
            </select>
        </p>
        <?php
        ...
        <p>
            ...
            <select name="<?php echo $this->get_field_name($name); ?>">
                <?php while (have_posts()) : the_post() ?>
                    <option <?php echo Utils::maybeSelected($post->ID == $selectedOption); ?> value="<?php echo $post->ID; ?>" title="<?php echo get_the_title(); ?>"><?php echo get_the_title(); ?></option>
                <?php endwhile; ?>
            </select>
            <!-- Reset Query-->
            <?php wp_reset_query(); ?>
    ...
    protected function formElementSelectCategory($name, $selectedOption, $desc)
    {
        ?>
        <p>
            <label for="<?php echo $this->get_field_id($name); ?>"><?php echo $this->translate($desc . ':'); ?></label> 
            ...
            <select name="<?php echo $this->get_field_name($name); ?>">
                <?php foreach ($cats as $category): ?>
                    <option <?php echo Utils::maybeSelected($category->cat_ID == $selectedOption); ?> value="<?php echo $category->cat_ID; ?>"><?php echo $category->name; ?></option>
                <?php endforeach; ?>
            </select>            
        </p>
        <?php
    ...
    protected function formElementSelectPostLinks($name, $selectedOption, $desc)
    {
        ...
        <p>
            ...
            <select name="<?php echo $this->get_field_name($name); ?>">
                <?php while (have_posts()) : the_post() ?>
                    <option <?php echo Utils::maybeSelected(get_permalink($post->ID) == $selectedOption); ?> value="<?php echo get_permalink($post->ID); ?>" title="<?php echo get_the_title(); ?>"><?php echo get_the_title(); ?></option>
                <?php endwhile; ?>
            </select>
            <!-- Reset Query-->
            <?php wp_reset_query(); ?>
    ...
    protected function formElementSelectPageLinks($name, $selectedOption, $desc)
    {
        ...
        <p>
            <label for="<?php echo $this->get_field_id($name); ?>"><?php echo $this->translate($desc . ':'); ?></label> 
            <select name="<?php echo $this->get_field_name($name); ?>"> 
                <option value=""><?php echo esc_attr(__('Select page', CHT_APP_TEXT_DOMAIN)); ?></option> 
                <?php $pages = get_pages(); ?>
                <?php foreach ($pages as $page): ?>
                    <option <?php echo Utils::maybeSelected(get_page_link($page->ID) == $selectedOption); ?> value="<?php echo get_page_link($page->ID); ?>"><?php echo $page->post_title; ?></option>
                <?php endforeach; ?>
            </select>
        </p>
        <?php
       ...
       ?>
        <p>
            ...
            <select name="<?php echo $this->get_field_name($name); ?>"> 
                ...
                <?php $pages = get_pages(); ?>
                <?php foreach ($pages as $page): ?>
                    <option <?php echo Utils::maybeSelected($page->ID == $selectedOption); ?> value="<?php echo get_page_link($page->ID); ?>"><?php echo $page->post_title; ?></option>
                <?php endforeach; ?>
            </select>
        </p>
        <?php
